% Source codes for reproducing the numerical experiments (Example 2) in 
%
...
parameters = [theta];
disp('Source code for reproducing the numerical experiment (Example 2, Table 2)'); 
...
disp('Compute: the post-array L such that L=QA and the derivative of L at the point theta = 2');
disp('         In the post-array L only the last main 3 by 3 block is lower triangular');
...
disp('Results');
disp('<press any key to run the algorithm and to see the result>');
disp('The computed post-array L (at the point theta = 2):');
[PostArray,Diff_PostArray,orth] = Diff_QL(PreArray,3,Diff_PreArray);
disp(PostArray);
disp('The orthogonal transformation (QA=L):');
disp(orth);
disp('The computed derivative of the post-array R (at the point theta = 2):');
disp(Diff_PostArray);
disp('________________________________________________________________________________________');
disp('1. Accuracy of factorization: norm( A^T*A - L^T*L ,Inf) ');
disp(norm(PreArray'*PreArray - PostArray'*PostArray,inf));

disp('2. Accuracy of derivative computation: norm( (A^T*A)^{\prime} - (L^T*L)^{\prime} ,Inf) ');
